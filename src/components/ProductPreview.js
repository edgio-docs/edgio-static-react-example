...
const ProductPreview = ({ name, path, images, prices, loading = false }) => {
  const nonSlashPath = path.replace(/\//g, '')
  return (
    <Link className="w-[250px] h-[250px] relative mt-2 border border-white p-1" to={`/product/${nonSlashPath}`}>
      ...
      {!loading && <img className="w-[250px] h-full object-cover" loading="lazy" width={1200} height={1200} src={relativizeURL(images[0].url)} />}
      {loading && <div className="h-full w-full bg-white/50 animate-pulse"></div>}
    </Link>
  )
}
